...
**GitHub comments:** When adding a pull request or issue comment in `chunfeng789/new-api`, always use the authenticated GitHub account `chunfeng789`. Before posting, verify the active account with `gh api user --jq .login`; if another account is active, switch with `gh auth switch --user chunfeng789`.
