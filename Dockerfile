...
# built index.html references that origin. The files themselves are uploaded by the
# upload_cdn job in .github/workflows/docker-build.yml (scripts/upload-cdn.mjs), which
# must run the same build command as below so the content hashes match.
ARG ASSET_PREFIX
